...
if prompt := st.chat_input(placeholder="What is this data about?"):
    ...
    deployment_id = "sparcgpt4" #"hclnewopenaicricket" #"sparcgpt4" 
    llm = AzureOpenAI(api_base=api_base,api_version = api_version,api_token = api_key, deployment_name = deployment_id,is_chat_model=True)
    sdf = SmartDataframe(data,config = {"llm": llm,
                                        "enable_cache": False,
                                        "conversational": True,
                                        "save_charts":True,
                                        # "save_charts_path":"C:\\Users\\mohanapriya.c\\Desktop\\Homepage",
                                        "custom_whitelisted_dependencies":["networkx","pgmpy","causalnex","os","causalgraphicalmodels"],
                                        "callback": StdoutCallback()})
    #sdf = SmartDataframe(data, config={"llm": llm})
    #print(sdf)
    #pandas_ai = PandasAI(llm,save_charts=True,save_charts_path ="C:/ReportingApp1/pandasai-app/pandasai_app",conversational=True,callback=StdoutCallback(),enable_cache = False,custom_whitelisted_dependencies=["networkx","pgmpy","causalnex"])
